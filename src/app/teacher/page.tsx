...
import { getGroupSchedule, getAreaSchedule, getAreaTypeBgLight } from '@/lib/schedule';
...
export default function TeacherPage() {
  ...
  // Sync data when changed in another tab (e.g., admin dashboard)
  useEffect(() => {
  ...
  const handleToggleAttendance = (kidId: string, sessionId: string) => {
    ...
    const newData = { ...data };
    for (const groupId of Object.keys(newData.groups)) {
      const kidIndex = newData.groups[groupId].kids.findIndex(k => k.id === kidId);
      if (kidIndex !== -1) {
        const kid = newData.groups[groupId].kids[kidIndex];
        const attended = kid.attended || [];
        if (attended.includes(sessionId)) {
          kid.attended = attended.filter(s => s !== sessionId);
        } else {
          kid.attended = [...attended, sessionId];
        }
        break;
      }
    }
  ...
  const markAllAttendance = (groupId: string, sessionIndex: number, markPresent: boolean) => {
    if (!data) return;

    const sessionId = `session-${sessionIndex + 1}`;
    const newData = { ...data };

    newData.groups[groupId].kids.forEach(kid => {
      const attended = kid.attended || [];
      if (markPresent && !attended.includes(sessionId)) {
        kid.attended = [...attended, sessionId];
      } else if (!markPresent && attended.includes(sessionId)) {
        kid.attended = attended.filter(s => s !== sessionId);
      }
    });
            ...
            {selectedArea && (
              <div className="robo-card p-6">
                ...
                <div className="space-y-4">
                  {getAreaSchedule(data, selectedArea).map((slot, sessionIdx) => {
                    const sessionId = `session-${sessionIdx + 1}`;
                    return (
                      <div key={sessionIdx} className="border border-[#00adb3]/20 rounded-xl p-4">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <span className="robo-badge">
                            {slot.groups.length} {slot.groups.length === 1 ? 'group' : 'groups'}
                          </span>
                        </div>
                              ...
                              <div key={groupId} className="bg-[#f0f7f7] rounded-xl p-4">
                                <div className="flex items-center justify-between mb-3">
                                  <h4 className="font-semibold text-[#003439]">
                                    Group {groupId} <span className="font-normal text-[#05575c]/60">({group.ageRange} yrs)</span>
                                  </h4>
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs text-[#05575c]/60">{attendedCount}/{group.kids.length}</span>
                                    <button
                                      onClick={() => markAllAttendance(groupId, sessionIdx, true)}
                                    ...
                                    <button
                                      onClick={() => markAllAttendance(groupId, sessionIdx, false)}
                                ...
                                <div className="space-y-1 max-h-48 overflow-y-auto">
                                  {group.kids.map(kid => {
                                    ...
                                    return (
                                      <button
                                        key={kid.id}
                                        onClick={() => handleToggleAttendance(kid.id, sessionId)}
            ...
            {Object.entries(data.groups).map(([groupId, group]) => (
              <div key={groupId} className="robo-card p-6">
                ...
                <div className="space-y-2 mb-4">
                  {getGroupSchedule(data, groupId).map((slot, idx) => {
                    const sessionId = `session-${idx + 1}`;
                    const attendedCount = group.kids.filter(k => (k.attended || []).includes(sessionId)).length;
                    return (
                      <details key={idx} className="group">
                        <summary
                          className={`p-3 rounded-xl border cursor-pointer transition-all hover:shadow-md ${getAreaTypeBgLight(slot.area.type)}`}
                        >
                        ...
                        <div className="mt-2 p-3 bg-white rounded-xl border border-gray-100">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => markAllAttendance(groupId, idx, true)}
                                className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors"
                              >
                                Mark All
                              </button>
                              <button
                                onClick={() => markAllAttendance(groupId, idx, false)}
                                className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors"
                              >
                                Clear
                              </button>
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-sm flex items-center justify-between p-2 rounded-lg transition-all ${
                                    isAttended
                                      ? 'bg-green-100 border-2 border-green-300'
                                      : 'bg-[#f0f7f7] border-2 border-transparent hover:border-gray-200'
                                  }`}
                                >
                                  <span className="text-[#003439] flex items-center gap-2">
                                    {kid.name}
