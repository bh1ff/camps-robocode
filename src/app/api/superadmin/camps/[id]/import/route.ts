...
export async function POST(
  ...
  try {
    ...
    const targetGroupSize = Math.ceil(records.length / 6);
    ...
    for (const age of sortedAges) {
      for (const kid of kidsByAge[age]) {
        const parts = kid.name.trim().split(/\s+/);
        ...
        if (currentGroupKids.length >= targetGroupSize && currentGroupIndex < 5) {
          const gName = groupNames[currentGroupIndex];
          groupsToCreate.push({
            name: gName,
            color: wristbandColors[gName] || 'blue',
